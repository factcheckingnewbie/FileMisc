...
static void write_node(const ordered_json& node, std::ostream& out, int parent_kind, int depth, int indent_size) {
    ...
    if (DSL::is_skip_category(cat)) {
        ...
        if (cat == DSL::CatComment) {
            ...
            out << indent_str(depth, indent_size) << raw << "\n";
        } else {
            out << "\n";
        }
        return;
    ...
    if (cat == DSL::CatProperty) {
        out << indent_str(depth, indent_size) << name << ":" << value << "\n";
        return;
    }

    // Separator
    if (cat == DSL::CatSeparator) {
        out << indent_str(depth, indent_size) << prefix << "\n";
        return;
    }
    ...
    if (node.contains("children") && node["children"].is_array()) {
        for (const auto& child : node["children"]) {
            write_node(child, out, kind, depth + 1, indent_size);
        }
    }
}

void JSON2DSLWriter::write_dsl(const ordered_json& root, std::ostream& out, int indent_size) const {
    write_node(root, out, DSL::Unknown, 0, indent_size);
}