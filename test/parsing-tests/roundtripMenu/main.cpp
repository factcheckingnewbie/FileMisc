...
Panel buttonbar position:top color:lightgray
    ...
    DSLGenerator generator;
    QString regeneratedDSL = generator.generate(json);
    
    qDebug().noquote() << "\nRegenerated DSL:";
    qDebug().noquote() << regeneratedDSL;
    
    // Save regenerated DSL
    QFile dslFile("output.dsl");
    if (dslFile.open(QIODevice::WriteOnly | QIODevice::Text)) {
        QTextStream stream(&dslFile);
        stream << regeneratedDSL;
        dslFile.close();
        qDebug() << "\nDSL saved to output.dsl";
    }
